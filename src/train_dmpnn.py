"""
Priority 3: train D-MPNN across both datasets and all three seeds, same
protocol as src/train.py.

Usage:
    python -m src.train_dmpnn --dataset bbbp --seed 0
    python -m src.train_dmpnn --run-all              # all 6 runs (2 datasets x 3 seeds)

Results land in results/dmpnn/<dataset>/seed<N>/, aggregated into
results/dmpnn/dmpnn_runs.csv -- same columns as results/all_runs.csv (minus
the `model` column, since there's only one) so it merges into the same
comparison table as GCN/SAGE/GIN/GAT.
"""
...
from .models_dmpnn import build_dmpnn_model, count_parameters
from .split import check_split, scaffold_split

RESULTS_DIR = ROOT / "results" / "dmpnn"
SEEDS = [0, 1, 2]
...
def train_dmpnn_one(
    ...
    model = build_dmpnn_model(hidden=hidden, depth=depth, dropout=dropout).to(dev)
    ...
    if out_dir is None:
        out_dir = RESULTS_DIR / dataset_name / f"seed{seed}"
...
def collect() -> pd.DataFrame:
    ...
    for path in sorted(RESULTS_DIR.glob("*/seed*/metrics.json")):
        ...
        rows.append({
            ...
            "test_f1": r["test"]["f1"], "test_mcc": r["test"]["mcc"],
        })
    return pd.DataFrame(rows)


def run_all(datasets=DATASET_NAMES, seeds=SEEDS, epochs=200, patience=30,
    ...
    for dataset in datasets:
        for seed in seeds:
            ...
            print(f"\n[{done}/{total}] dmpnn {dataset}/seed{seed}")
            try:
                train_dmpnn_one(dataset_name=dataset, seed=seed, epochs=epochs,
    ...
    df.to_csv(RESULTS_DIR / "dmpnn_runs.csv", index=False)
    ...
    df.groupby(["dataset", "model"])[metrics].agg(["mean", "std"]).round(4).to_csv(
        RESULTS_DIR / "dmpnn_summary.csv"
    )
    print(f"\nwrote {RESULTS_DIR / 'dmpnn_runs.csv'} ({len(df)} runs)")
    if failed:
...
def main() -> None:
    ...
    if not args.dataset:
        p.error("either --run-all, or --dataset")

    print(f"[dmpnn {args.dataset}/seed{args.seed}]")
    train_dmpnn_one(dataset_name=args.dataset, seed=args.seed, epochs=args.epochs,
