...
# Kept OUT of MODELS deliberately. GINEConv is the only operator here that
# consumes edge_attr, so adding it to the core comparison would confound the
# aggregation-operator comparison with an input-signal advantage -- exactly
# the reason README.md gives for why GIN/GAT don't get edge features either.
# Run it as its own single-model ablation instead:
#   python -m src.train --model gine --dataset bbbp --seed 0
ABLATION_MODELS = ["gine"]
...
def _make_conv(kind: str, in_dim: int, out_dim: int, heads: int) -> nn.Module:
    ...
    if kind == "gine":
        # GINEConv requires edge_dim so it can project edge_attr to match the
        # node feature width before adding it into the message.
        mlp = nn.Sequential(
            nn.Linear(in_dim, out_dim), nn.ReLU(), nn.Linear(out_dim, out_dim)
        )
    ...
    raise ValueError(
        f"unknown conv type: {kind!r} (expected one of {MODELS + ABLATION_MODELS})"
    )
...
def _shape_check() -> None:
    ...
    for kind in MODELS + ABLATION_MODELS:
